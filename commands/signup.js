...
if (message.channel.type === "dm") {
		Data.findOne({
			...
			Data.findOne({ 'displayName': reason }, (err1, DataBOI1) => {
				if (DataBOI1) {
					if (DataBOI1.displayName == reason.toString()) {
						ReadyBOI = false
						return message.channel.send('Username is already in use please try again (run a!setup-user again)')
					}
				}else{
          ...
          const embed = new Discord.MessageEmbed()
						...
						.addFields(
						{ name: 'Email', value: reason + "@aurorafn.dev"},
